...
from IngresarAjustes import ajustes
...
def mostrar_libro_mayor(frame_menu, ventana):
    ...
    # Título
    Contorno(canvas, texto="LIBRO MAYOR CON AJUSTES", x=ancho // 2, y=ancho - (ancho - 50), fuente=("Minecraftia", 50, "bold"),
    ...
    def mostrar_cuenta(i):
        ...
        # Crear una lista combinada de movimientos y ajustes para esta cuenta
        registros_cuenta = []

        # Agregar movimientos de esta cuenta
        ...
        # Agregar ajustes de esta cuenta (solo si existe el arreglo ajustes)
        if 'ajustes' in globals() and ajustes:
            for ajuste in ajustes:
                for cta, debe, haber in ajuste['datos']:
                    if cta == cuenta:
                        registros_cuenta.append({
                            'numero': f"AJ-{ajuste['numero']}",
                            'descripcion': f"AJUSTE: {ajuste['descripcion']}",
                            'debe': debe if debe else 0,
                            'haber': haber if haber else 0,
                            'tipo': 'ajuste'
                        })

        # Procesar todos los registros de esta cuenta
        for registro in registros_cuenta:
            if registro['debe'] > 0:
                saldo += registro['debe']
                total_debe += registro['debe']
                saldo_mostrar = abs(saldo) if saldo < 0 else saldo
                tag = 'evenrow' if fila_par else 'oddrow'
                tabla.insert("", "end", values=(
                    registro['numero'], registro['descripcion'], f"{registro['debe']:.2f}", "", f"{saldo_mostrar:.2f}"
                ), tags=(tag,))
                fila_par = not fila_par
            elif registro['haber'] > 0:
                saldo -= registro['haber']
                total_haber += registro['haber']
                saldo_mostrar = abs(saldo) if saldo < 0 else saldo
                tag = 'evenrow' if fila_par else 'oddrow'
                tabla.insert("", "end", values=(
                    registro['numero'], registro['descripcion'], "", f"{registro['haber']:.2f}", f"{saldo_mostrar:.2f}"
                ), tags=(tag,))
                fila_par = not fila_par
